...
async function setupDatabase() {
  ...
  try {
    ...
    await client.query(setupRLSSQL);
    console.log('✅ RLS policies set up successfully');

    // Verify tables exist
    const tables = ['contacts', 'job_applications', 'community_requests', 'resource_enquiries', 'admins', 'site_settings'];
    ...
    if (error.code === 'ENOTFOUND') {
      console.error('💡 Hint: The hostname could not be resolved. This might be a DNS issue or the project reference is incorrect.');
    }
  } finally {
    await client.end();
  }
